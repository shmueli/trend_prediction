...
def simulate():
    ...
    #end = datetime.strptime('20200801000000', constants.GENERAL_DATE_FORMAT)
    end = datetime.strptime('20210101000000', constants.GENERAL_DATE_FORMAT)
    ...
    time = start
    while time<end:
        time_str = time.strftime(constants.GENERAL_DATE_FORMAT)
        ...
        if len(winners) > 0:
            ...
            nAddedEdges = draw_added_edges_for_existing_nodes(g, winners)
            ...
            added_edges.append( float(nAddedEdges)/float(g.number_of_edges()))
        else:
            added_edges.append(0)
        ...
        nRemovedEdges = draw_removed_edges_for_existing_nodes(loosers)
    ...
    visualize.plot_changes(times, added_edges, 'temp_added_edges', False)
    #'''
...
def draw_added_edges_for_existing_nodes(g, winners):
    in_degree = 0
    for v in winners:
        in_degree = in_degree + g.in_degree(v)
        
    return 5*(in_degree+1)
    #return 0


def draw_removed_edges_for_existing_nodes(loosers):
    #return 4*len(loosers)
    return 0
