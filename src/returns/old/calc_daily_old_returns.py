...
from graphs import load_graphs_old
...
def execute():
    for p in constants.PERIODS:
        store_daily_returns_for_all_individuals(constants.PERIODS[p]['start'], constants.PERIODS[p]['end'], p)


def store_daily_returns_for_all_individuals(start, end, period):
    output_filename = constants.RETURNS_FOLDER_NAME + 'daily_old_returns' + '_' + str(period)
    ...
    while time<end:
        ...
        for v in g_trades.node:
            r = calc_returns_for_individual(g_trades, v, time, tomorrow(time), period)
...
def calc_returns_for_individual(g, v, open_time, close_time, period):
    ...
    if g.has_node(v):
        for e in g.out_edges(v, data=True):
            for t in e[2]['data']:
                ...
                profit, invested = calc_return_for_trade(t, adj_open_time, adj_close_time, timedelta(hours=1), period)
...
def calc_return_for_trade(t, open_time, close_time, accuracy, period):    
    ...
    if float(close_rate)/float(open_rate)>float(2) or float(open_rate)/float(close_rate)>float(2):
        open_rate = load_rates.get_rate(t['buyCurAbbreviation'], t['sellCurAbbreviation'], open_time, accuracy, period)
        close_rate = load_rates.get_rate(t['buyCurAbbreviation'], t['sellCurAbbreviation'], close_time, accuracy, period)        
...
def get_usd_rate(close_time, target, source, mediator, period):
    ...
    if instrument not in load_rates.get_instruments(period):
        buy = source
        sell = target    
        instrument = buy + '/' + sell
